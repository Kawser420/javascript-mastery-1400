...
(window as any).beginnerBasicsSolvers = {
    ...
    'sum-two-numbers': ({ num1, num2 }: { num1: any, num2: any }) => parseNumber_BB(num1) + parseNumber_BB(num2),
    'subtract-numbers': ({ num1, num2 }: { num1: any, num2: any }) => parseNumber_BB(num1) - parseNumber_BB(num2),
    'simple-multiplication': ({ num1, num2 }: { num1: any, num2: any }) => parseNumber_BB(num1) * parseNumber_BB(num2),
    ...
    'find-remainder': ({ dividend, divisor }: { dividend: any, divisor: any }) => parseNumber_BB(dividend) % parseNumber_BB(divisor),
    'exponentiation': ({ base, exponent }: { base: any, exponent: any }) => Math.pow(parseNumber_BB(base), parseNumber_BB(exponent)),
    ...
    'is-greater-than': ({ num1, num2 }: { num1: any, num2: any }) => parseNumber_BB(num1) > parseNumber_BB(num2),
    'logical-and': ({ age, hasLicense }: { age: any, hasLicense: any }) => parseNumber_BB(age) >= 18 && parseBoolean_BB(hasLicense),
    'logical-or': ({ isAdmin, isOwner }: { isAdmin: any, isOwner: any }) => parseBoolean_BB(isAdmin) || parseBoolean_BB(isOwner),
    'logical-not': ({ value }: { value: any }) => !parseBoolean_BB(value),
    'check-is-even': ({ num }: { num: any }) => parseNumber_BB(num) % 2 === 0,
    'simple-if-else': ({ age }: { age: any }) => parseNumber_BB(age) >= 18 ? 'Can vote' : 'Cannot vote',
    ...
    'ternary-operator-check': ({ isLoggedIn }: { isLoggedIn: any }) => parseBoolean_BB(isLoggedIn) ? 'Welcome, User!' : 'Please log in.',
    ...
    'max-of-three': ({ num1, num2, num3 }: { num1: any, num2: any, num3: any }) => Math.max(parseNumber_BB(num1), parseNumber_BB(num2), parseNumber_BB(num3)),
    ...
    'get-array-length': ({ arr }: { arr: string }) => parseStrArray_BB(arr).length,
    ...
    'area-of-rectangle': ({ width, height }: { width: any, height: any }) => parseNumber_BB(width) * parseNumber_BB(height),
    'celsius-to-fahrenheit': ({ celsius }: { celsius: any }) => (parseNumber_BB(celsius) * 9/5) + 32,
    'fahrenheit-to-celsius': ({ fahrenheit }: { fahrenheit: any }) => (parseNumber_BB(fahrenheit) - 32) * 5/9,
    ...
    'string-slicing': ({ str, start, end }: { str: string, start: any, end: any }) => str.slice(parseNumber_BB(start), parseNumber_BB(end)),
    'array-join': ({ arr, separator }: { arr: string, separator: string }) => parseStrArray_BB(arr).join(separator),
