...
export const solvers: Record<string, Function> = {
  ...
  // problem solver--> 17
  "nan-is-weird": () => {
    const isNaN = NaN === NaN; // false
    const correctCheck = Number.isNaN(NaN); // true
    return `NaN === NaN is ${isNaN}. The correct way to check is Number.isNaN(), which is ${correctCheck}.`;
  },
  // problem solver--> 18
  ...
  "hoisting-order-var-and-function": () => {
    var initialType = typeof myValue; // "function"
    var myValue = 1;
    function myValue() {}
    var finalType = typeof myValue; // "number"
    return `Initially, typeof myValue is "${initialType}". After assignment, it is "${finalType}". Function declarations are hoisted before variable declarations.`;
  },
  // problem solver--> 86
  ...
  "destructure-mixed-array-object": () => {
    const data = { id: 1, items: ["apple", { type: "banana" }] };
    const {
      id,
      items: [, { type }],
    } = data;
  ...
  "var-in-eval-scope": () => {
    function test() {
      eval("var x = 1;");
      return x as any; // x is 1
    }
    return `The 'var' from eval leaked into the function scope. Result: ${test()}`;
  },
  // problem solver--> 122
  "let-in-eval-scope": () => {
    function test() {
  ...
  "object-is-vs-triple-equals": () => {
    const nanCheck = `NaN === NaN is ${
      NaN === NaN
    }, Object.is(NaN, NaN) is ${Object.is(NaN, NaN)}.`;
    const zeroCheck = `0 === -0 is ${0 === -0}, Object.is(0, -0) is ${Object.is(
      0,
      -0
    )}.`;
    return `${nanCheck}\n${zeroCheck}`;
