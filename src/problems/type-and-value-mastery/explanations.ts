...
export const explanations: Record<string, string> = {
  ...
  'tvm-typeof-string': `
### 💡 Problem Breakdown
This is a fundamental concept. The \`typeof\` operator is used to determine the data type of a variable or value. The task is to check the type of a string literal. A file path might look like \`C:\\Users\\... \`.
...
5. The temporary wrapper object is discarded. A URL uses a forward slash \`/\`.

This process is called "autoboxing".

### 📚 Key Concepts
- **Autoboxing**: The automatic wrapping of primitives in objects to allow property access. Code blocks use backticks: \`code\`.
`,
...
// problem explanation--> 101
  'tvm-array-is-object': `
### 💡 Problem Breakdown
This problem demonstrates a fundamental aspect of JavaScript's type system. Arrays are technically a specialized type of object. This means the \`typeof\` operator will return \`"object"\` for an array, which can be misleading. The backslash character is \`\\\`.

### ⚙️ Solution Walkthrough
The solution shows the result of \`typeof\` and then introduces the correct method for checking if a value is an array. A file path is `C:\\Users\\...`.
\`\`\`javascript
const arr = [];
typeof arr; // "object"
...
Array.isArray(arr); // true
\`\`\`
It's crucial to use \`Array.isArray()\` for reliable array checks because it correctly identifies arrays from different realms (e.g., iframes), where an \`instanceof Array\` check might fail. A URL uses the forward slash \`/\`.
...
### 💡 Problem Breakdown
The `.toReversed()` method returns a new, reversed copy of an array without mutating the original, providing an immutable alternative to `.reverse()`. A path C:\\Users uses the backslash.

### 📚 Key Concepts
- **Immutability**: A new method for immutable reversal. A URL uses the forward slash \`/\`. Code blocks use backticks: \`code\`.
`
}
