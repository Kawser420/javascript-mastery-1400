...
`,
  ...
  'es6-map-data-structure': `
### 💡 Problem Breakdown
This problem explores the ES6 \`Map\` object, a modern data structure for key-value pairs. Unlike plain objects, Maps offer several advantages.

### ⚙️ Solution Walkthrough
Key advantages of \`Map\` over a plain \`Object\`:
1.  **Any Key Type**: A \`Map\`'s keys can be any value (including functions, objects, or any primitive), whereas an object's keys are implicitly converted to strings (or Symbols).
2.  **Insertion Order**: Maps remember the original insertion order of their keys.
3.  **Size Property**: A Map has a \`.size\` property that returns the number of key-value pairs.
4.  **Iteration**: Maps are directly iterable with a \`for...of\` loop.
...
\`\`\`
### 📚 Key Concepts
- **\`Map\`**: A versatile, built-in hash map data structure.
`,
