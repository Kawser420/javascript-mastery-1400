...
export const solvers: Record<string, Function> = {
  ...
  "sum-two-numbers": ({ num1, num2 }: { num1: any; num2: any }) => parseNumber(num1) + parseNumber(num2),
  "subtract-numbers": ({ num1, num2 }: { num1: any; num2: any }) => parseNumber(num1) - parseNumber(num2),
  "simple-multiplication": ({ num1, num2 }: { num1: any; num2: any }) => parseNumber(num1) * parseNumber(num2),
  ...
  "find-remainder": ({ dividend, divisor }: { dividend: any; divisor: any }) => parseNumber(dividend) % parseNumber(divisor),
  "exponentiation": ({ base, exponent }: { base: any; exponent: any }) => Math.pow(parseNumber(base), parseNumber(exponent)),
  ...
  "get-first-character": ({ str }: { str: string }) => str ? str[0] : "String is empty",
  "get-last-character": ({ str }: { str: string }) => str ? str[str.length - 1] : "String is empty",
  "concatenate-strings": ({ str1, str2 }: { str1: string; str2: string }) => str1 + str2,
  ...
  "get-data-type": ({ value }: { value: string }) => {
    if (!isNaN(Number(value)) && value.trim() !== "" && !/^\s*$/.test(value)) return "number";
  ...
  "strict-equality": ({ val1, val2 }: { val1: any; val2: any }) => {
    ...
    return `Comparing '${val1}' (${typeof n1}) and '${val2}' (${typeof n2}) with === results in: ${n1 === n2}`;
  },
  "loose-equality": ({ val1, val2 }: { val1: any; val2: any }) => `Comparing '${val1}' and '${val2}' with == results in: ${val1 == val2}`,
  "is-greater-than": ({ num1, num2 }: { num1: any; num2: any }) => parseNumber(num1) > parseNumber(num2),
  "logical-and": ({ age, hasLicense }: { age: any; hasLicense: any }) => parseNumber(age) >= 18 && parseBoolean(hasLicense),
  "logical-or": ({ isAdmin, isOwner }: { isAdmin: any; isOwner: any }) => parseBoolean(isAdmin) || parseBoolean(isOwner),
  "logical-not": ({ value }: { value: any }) => !parseBoolean(value),
  "check-is-even": ({ num }: { num: any }) => parseNumber(num) % 2 === 0,
  "simple-if-else": ({ age }: { age: any }) => parseNumber(age) >= 18 ? "Can vote" : "Cannot vote",
  ...
  "ternary-operator-check": ({ isLoggedIn }: { isLoggedIn: any }) => parseBoolean(isLoggedIn) ? "Welcome, User!" : "Please log in.",
  "is-leap-year": ({ year }: { year: any }) => {
    const y = parseNumber(year);
    if ((y % 4 === 0 && y % 100 !== 0) || y % 400 === 0) return `${y} is a leap year.`;
    return `${y} is not a leap year.`;
  },
  "max-of-three": ({ num1, num2, num3 }: { num1: any; num2: any; num3: any }) => Math.max(parseNumber(num1), parseNumber(num2), parseNumber(num3)),
  ...
  "string-includes": ({ mainStr, subStr }: { mainStr: string; subStr: string; }) => mainStr.includes(subStr),
  "area-of-rectangle": ({ width, height }: { width: any; height: any }) => parseNumber(width) * parseNumber(height),
  "celsius-to-fahrenheit": ({ celsius }: { celsius: any }) => (parseNumber(celsius) * 9) / 5 + 32,
  "fahrenheit-to-celsius": ({ fahrenheit }: { fahrenheit: any }) => ((parseNumber(fahrenheit) - 32) * 5) / 9,
  "check-null": ({ value }: { value: string }) => value.toLowerCase().trim() === "null",
  "check-undefined": ({ value }: { value: string }) => value.toLowerCase().trim() === "undefined",
  "string-repeat": ({ str, count }: { str: string; count: any }) => {
    const c = parseNumber(count);
    if (c < 0) throw new Error("Count cannot be negative.");
    return str.repeat(c);
  },
  "string-slicing": ({ str, start, end }: { str: string; start: any; end: any; }) => str.slice(parseNumber(start), parseNumber(end)),
  "array-join": ({ arr, separator }: { arr: string; separator: string }) => parseStrArray(arr).join(separator),
  ...
  "random-integer-in-range": ({ min, max }: { min: any; max: any }) => {
    ...
    if (minVal > maxVal) throw new Error("Min value cannot be greater than max value.");
    return Math.floor(Math.random() * (maxVal - minVal + 1)) + minVal;
  },
  "string-starts-with": ({ str, subStr }: { str: string; subStr: string; }) => str.startsWith(subStr),
  "string-trim": ({ str }: { str: string }) => `"${str.trim()}"`,
  "string-replace-first": ({ str, target, replacement }: { str: string; target: string; replacement: string; }) => str.replace(target, replacement),
  ...
  "array-slice": ({ arr, start, end }: { arr: string; start: any; end: any; }) => {
    const array = parseStrArray(arr);
  ...
  "string-last-index-of": ({ str, subStr }: { str: string, subStr: string }) => str.lastIndexOf(subStr),
