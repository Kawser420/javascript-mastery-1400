    ...
    const nodes = parts.map((part, index) => {
      if (part.startsWith("```")) {
        ...
        const html = part
          ...
          .replace(
            /`([^`]+)`/g,
            '<code class="bg-base-300 px-1 py-0.5 rounded-md text-sm">$1</code>'
          )
          .replace(/\n/g, "<br />");
      ...
      if (problem) {
        ...
        if (!solverFunction) {
          parseAndSetExplanation(
            '<div class="text-error">Could not find solver function for this problem.</div>'
          );
          setIsLoading(false);
          return;
        }
        ...
        getAIExplanation(problem, solutionCode)
          ...
          .catch((err) => {
            console.warn(
              "Live AI explanation failed, falling back to static version. Error:",
              err.message
            );
            ...
            if (fallbackExplanation) {
              ...
              parseAndSetExplanation(
                `<div class="alert alert-error">Error: AI explanation failed and no fallback is available.</div>`
              );
            }
          })
      ...
      <div className="modal-box w-11/12 max-w-4xl">
        ...
        <div className="py-4">
          {isLoading ? (
            ...
            <div className="prose max-w-none">{explanationParts}</div>
          )}
        </div>
