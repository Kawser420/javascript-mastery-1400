...
const ProblemCard: React.FC<ProblemCardProps> = ({ problem, onExplain }) => {
  ...
  const solveProblem = (problemId: string, currentInputs: Record<string, string>) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const finalInputs = problem.inputs.reduce((acc, input) => {
        acc[input.id] = inputs[input.id] || input.defaultValue?.toString() || '';
        return acc;
  ...
  return (
    <div className="card bg-base-100 shadow-xl transition-transform duration-300 hover:-translate-y-1 hover:shadow-2xl flex flex-col">
      <div className="card-body flex-grow">
        ...
        <p className="text-base-content/70 text-sm mb-4">{problem.description}</p>
       ...
       <div className="card-actions justify-end p-4 pt-0">
          {result && (
            <div className={`w-full result-alert mt-4 mb-2 p-3 rounded-lg text-sm ${ result.isSuccess ? "bg-success/20 text-success-content" : "bg-error/20 text-error-content" }`}>
              <span className="font-bold">{result.isSuccess ? "Result:" : "Error:"}</span>
              <span className="whitespace-pre-wrap break-words ml-2 font-mono">{result.message}</span>
            </div>
          )}
