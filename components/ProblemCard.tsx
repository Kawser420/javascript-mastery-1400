  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const dummyInputs = problem.inputs.reduce((acc, input) => {
          acc[input.id] = input.defaultValue?.toString() ?? (input.type === 'number' ? '0' : '');
          if (input.placeholder?.includes(',')) acc[input.id] = input.placeholder; // A heuristic for array inputs
          return acc;
      }, {} as Record<string, string>);

      const finalInputs = problem.inputs.every(i => inputs[i.id]?.trim() !== '') ? inputs : dummyInputs;
    ...
    <div className="card bg-base-100 shadow-xl transition-transform duration-300 hover:-translate-y-1 hover:shadow-2xl">
      <div className="card-body">
        ...
        <div className="card-actions justify-end mt-4">
            <button className="btn btn-outline btn-secondary" onClick={() => onExplain(problem)} disabled={isSolving}>
                Explain with AI
            </button>
            <button className="btn btn-primary" onClick={handleSubmit} disabled={isSolving}>
              {isSolving ? <span className="loading loading-spinner"></span> : problem.buttonText}
            </button>
        </div>
        {result && (
          <div className={`result-alert mt-4 alert ${result.isSuccess ? 'alert-success' : 'alert-error'}`}>
